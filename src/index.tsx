...
/* `__allyFP` is the debug handle cdp.py drives from an SSH session; see docs/DESIGN.md.
 *
 * `securitystore` is deliberately NOT declared here. @decky/ui already declares it, but
 * only as `{ IsLockScreenActive(): boolean }` -- it is a mobx store internal to the Steam
 * UI bundle and nobody has typed the rest. Redeclaring it collides (TS2717). The methods
 * this plugin needs beyond that one are reached through `stores()`, which widens to
 * `any` in a single place rather than casting at every call site. */
declare global {
  interface Window {
    __allyFP?: any;
  }
...
const backend = {
  verify: callable<[timeoutSeconds: number], any>("verify"),
  cancel: callable<[], any>("cancel"),
...
function stores(): any {
  const sec: any = window.securitystore;
...
let _koCached = null;
function isKorean() {
  if (_koCached !== null) return _koCached;
  try {
...
const lockUI = {
  ...
  mount() {
    try {
      ...
      if (this.el && this.doc === doc && doc.body.contains(this.el)) return true;
      this.unmount();
      this.doc = doc;
      ...
      const shade = (v) => {
        if (this._onPress) this.el.style.background = "rgba(255,255,255," + v + ")";
      };
...
const watcher = {
  ...
  offerRearm(gen, coolS) {
    const show = () => {
      if (gen !== this.generation || !isLocked()) return;
    ...
    this.countdown = setInterval(() => {
      if (gen !== this.generation || !isLocked()) return this.clearCountdown();
      left -= 1;
      if (left > 0) return lockUI.setState("cooling", { arg: left });
      this.clearCountdown();
      show();
    }, 1000);
  },
  ...
  async beginVerify() {
    ...
    try {
      while (gen === this.generation && isEnabled() && isLocked()) {
        ...
        if (gen !== this.generation || !isLocked()) break;
        ...
        if (res && res.result === "cancelled") break;
...
// ---- UI ------------------------------------------------------------------------
function Content() {
  const [enabled, setEnabledState] = SP.useState(isEnabled());
  const [status, setStatus] = SP.useState(null);
  const [busy, setBusy] = SP.useState(false);

  const refresh = SP.useCallback(async () => {
    setBusy(true);
    try {
  ...
  const row = (label, value) =>
    SP.createElement(
      DFL.PanelSectionRow,
      { key: label },
      SP.createElement(DFL.Field, { label: label, focusable: false, bottomSeparator: "none" }, String(value))
    );
  ...
  return SP.createElement(
    ...
    SP.createElement(
      ...
      SP.createElement(
        DFL.PanelSectionRow,
        null,
        SP.createElement(DFL.ToggleField, {
          label: "Fingerprint unlock",
          description:
            "OFF by default: this sensor has returned a match with no finger present. " +
    ...
    SP.createElement(
      ...
      SP.createElement(
        ...
        SP.createElement(
          DFL.ButtonItem,
          { layout: "below", disabled: busy, onClick: refresh },
          "Refresh"
        )
      )
    ),
      ...
      SP.createElement(
        ...
        SP.createElement(
          DFL.ButtonItem,
          {
            ...
            onClick: () => {
              const sec = stores();
              if (sec) sec.SetActiveLockScreenProps({ preventCancel: false });
            },
          },
...
export default definePlugin(() => {
  ...
  return {
    name: manifest.name,
    titleView: SP.createElement("div", { className: DFL.staticClasses.Title }, "Fingerprint Unlock"),
    content: SP.createElement(Content, null),
    icon: SP.createElement("div", null, "◉"),
    onDismount() {
      watcher.stop();
      try {
